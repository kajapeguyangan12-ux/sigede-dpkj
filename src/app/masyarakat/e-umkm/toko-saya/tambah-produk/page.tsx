...
function TambahProdukContent() {
    ...
    try {
      ...
      if (umkmDoc.exists()) {
        ...
        if (data.status !== 'aktif') {
          alert('UMKM harus disetujui terlebih dahulu sebelum menambah produk');
          router.push('/masyarakat/e-umkm/toko-saya');
          return;
        }
        if (data.userId !== user?.uid) {
          alert('Anda tidak memiliki akses ke UMKM ini');
          router.push('/masyarakat/e-umkm/toko-saya');
          return;
        }
        setUmkmData({ id: umkmDoc.id, ...data });
      } else {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length + formData.fotoProduk.length > 5) {
  ...
  const validateForm = () => {
    ...
    if (formData.fotoProduk.length === 0) {
      newErrors.fotoProduk = 'Minimal 1 foto produk harus diupload';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'produk-umkm'), productData);
        ...
        <div className="relative">
          ...
          <div className="relative z-10 flex items-center justify-between gap-4 px-6 py-5">
            ...
            <div className="flex-1 text-center">
              <h1 className="text-xl sm:text-2xl font-bold text-white drop-shadow-lg">Tambah Produk</h1>
              <p className="text-red-100 text-xs sm:text-sm mt-1 drop-shadow">{umkmData?.namaUsaha || 'Sistem Informasi Desa'}</p>
            </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <form onSubmit={handleSubmit} className="bg-white rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-red-600 to-pink-700 px-8 py-6">
            ...
            <p className="text-red-100 mt-2">Lengkapi semua informasi produk dengan benar</p>
          </div>
          ...
          <div className="px-8 py-6 bg-gray-50 flex gap-4">
            <Link href="/masyarakat/e-umkm/toko-saya" className="flex-1">
              <button
                type="button"
