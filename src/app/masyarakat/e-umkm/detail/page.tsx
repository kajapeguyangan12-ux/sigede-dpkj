...
import { recordVisit, addOrUpdateRating, getUserRatingForUMKM, getUMKMRatings, getRatingStatistics, type Rating } from '@/lib/ratingService';
...
function DetailTokoContent() {
  ...
  const trackVisit = async () => {
    try {
      if (!umkmId || !umkmData || !user) return;
      
      // Update visit count in UMKM doc
      ...
      // Record visit for rating reminder
      await recordVisit(user.uid, umkmId, umkmData.namaUsaha);
  ...
  const loadRatings = async () => {
    try {
      ...
      if (user) {
        const userRatingData = await getUserRatingForUMKM(user.uid, umkmId);
        setUserRating(userRatingData);
        if (userRatingData) {
          setRatingForm({
            rating: userRatingData.rating,
            kualitasProduk: userRatingData.kualitasProduk,
            pelayanan: userRatingData.pelayanan,
            harga: userRatingData.harga,
            kebersihan: userRatingData.kebersihan,
            ulasan: userRatingData.ulasan || ''
          });
        }
      }
      ...
      const stats = await getRatingStatistics(umkmId);
  ...
  const handleSubmitRating = async () => {
    try {
      ...
      // Calculate average from all aspect ratings
      const avgRating = (
        ratingForm.kualitasProduk +
        ratingForm.pelayanan +
        ratingForm.harga +
        ratingForm.kebersihan
      ) / 4;
      ...
      await addOrUpdateRating({
        ...
        rating: avgRating,
        kualitasProduk: ratingForm.kualitasProduk,
        pelayanan: ratingForm.pelayanan,
        harga: ratingForm.harga,
        kebersihan: ratingForm.kebersihan,
        ulasan: ratingForm.ulasan,
        helpful: 0
      });
      
      alert(userRating ? 'Rating berhasil diupdate!' : 'Terima kasih atas rating Anda!');
      setShowRatingModal(false);
      
      // Reload ratings
      await loadRatings();
      
      // Reload UMKM data to get updated average rating
      await fetchTokoData();
  ...
  const renderStarInput = (value: number, onChange: (val: number) => void, label?: string) => {
    return (
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="ml-2 text-sm text-gray-600">{value > 0 ? `${value}/5` : 'Belum dinilai'}</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-pink-50 to-red-50 pb-32">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 xl:px-10">
          ...
          <div className="p-4 sm:p-5 md:p-6">
            ...
            {user && (
              <button
                ...
                className="w-full mt-3 px-6 py-3 sm:py-3.5 bg-gradient-to-r from-yellow-400 to-yellow-500 hover:from-yellow-500 hover:to-yellow-600 text-gray-900 font-bold text-sm sm:text-base rounded-xl transition-all shadow-lg flex items-center justify-center gap-2"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
                {userRating ? 'Edit Rating & Ulasan' : 'Beri Rating & Ulasan'}
              </button>
            )}
          </div>
        </div>
        ...
        {/* Ratings Section */}
        {allRatings.length > 0 && (
          <div className="bg-white rounded-3xl p-4 sm:p-5 md:p-6 shadow-lg mb-6">
              ...
              <div className="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-2xl p-4 mb-4">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    {renderStarRating(Math.round(ratingStats.averageRating), 'md')}
                    <div className="text-sm text-gray-600 mt-1">{ratingStats.totalRatings} ulasan</div>
                  </div>
                      ...
                      <div key={star} className="flex items-center gap-2 text-xs">
                        ...
                        <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                          <div 
                            ...
                            style={{ 
                              width: `${((ratingStats.distribution[star as 1|2|3|4|5] || 0) / ratingStats.totalRatings) * 100}%` 
                            }}
                          />
                        </div>
                        <span className="w-8 text-right">{ratingStats.distribution[star as 1|2|3|4|5] || 0}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center text-xs mt-4">
                  <div className="bg-white rounded-lg p-2">
                    <div className="font-bold text-red-600">{ratingStats.averageKualitas.toFixed(1)}</div>
                    <div className="text-gray-600">Kualitas</div>
                  </div>
                  <div className="bg-white rounded-lg p-2">
                    <div className="font-bold text-red-600">{ratingStats.averagePelayanan.toFixed(1)}</div>
                    <div className="text-gray-600">Pelayanan</div>
                  </div>
                  <div className="bg-white rounded-lg p-2">
                    <div className="font-bold text-red-600">{ratingStats.averageHarga.toFixed(1)}</div>
                    <div className="text-gray-600">Harga</div>
                  </div>
                  <div className="bg-white rounded-lg p-2">
                    <div className="font-bold text-red-600">{ratingStats.averageKebersihan.toFixed(1)}</div>
                    <div className="text-gray-600">Kebersihan</div>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="space-y-4">
              {allRatings.slice(0, 5).map((rating) => (
                <div key={rating.id} className="border-b border-gray-200 pb-4 last:border-b-0">
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-red-400 to-pink-500 flex items-center justify-center text-white font-bold flex-shrink-0">
                      {rating.userName?.charAt(0).toUpperCase() || 'U'}
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-semibold text-gray-900">{rating.userName || 'Pengguna'}</span>
                        <span className="text-xs text-gray-500">
                          {rating.createdAt?.toDate?.().toLocaleDateString('id-ID') || ''}
                        </span>
                      </div>
                      {renderStarRating(Math.round(rating.rating), 'sm')}
                      {rating.ulasan && (
                        <p className="text-sm text-gray-700 mt-2">{rating.ulasan}</p>
                      )}
                    </div>
                  </div>
                </div>
      ...
      {showRatingModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-3 sm:p-4">
          <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            {/* Modal Header */}
            <div className="sticky top-0 bg-gradient-to-r from-yellow-400 to-orange-500 text-gray-900 p-4 sm:p-5 md:p-6 rounded-t-3xl z-10">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-800 text-xs sm:text-sm">{umkmData?.namaUsaha}</p>
                  </div>
                </div>
                <button
            ...
            <div className="p-4 sm:p-5 md:p-6 space-y-5">
              ...
              <div className="space-y-4">
                <div className="bg-gradient-to-r from-red-50 to-orange-50 rounded-2xl p-4">
                  ...
                  {renderStarInput(
                    ratingForm.kualitasProduk,
                    (val) => setRatingForm({...ratingForm, kualitasProduk: val}),
                    '⭐ Kualitas Produk'
                  )}
                  ...
                  {renderStarInput(
                    ratingForm.pelayanan,
                    (val) => setRatingForm({...ratingForm, pelayanan: val}),
                    '😊 Pelayanan'
                  )}
                  ...
                  {renderStarInput(
                    ratingForm.harga,
                    (val) => setRatingForm({...ratingForm, harga: val}),
                    '💰 Harga'
                  )}
                  ...
                  {renderStarInput(
                    ratingForm.kebersihan,
                    (val) => setRatingForm({...ratingForm, kebersihan: val}),
                    '✨ Kebersihan'
                  )}
                </div>
                
                {/* Overall Rating Display */}
                {(ratingForm.kualitasProduk + ratingForm.pelayanan + ratingForm.harga + ratingForm.kebersihan) > 0 && (
                  <div className="bg-yellow-50 border-2 border-yellow-200 rounded-2xl p-4 text-center">
                    <div className="text-sm text-gray-600 mb-1">Rating Keseluruhan</div>
                    <div className="text-3xl font-bold text-gray-900">
                      {((ratingForm.kualitasProduk + ratingForm.pelayanan + ratingForm.harga + ratingForm.kebersihan) / 4).toFixed(1)}
                    </div>
                    {renderStarRating(Math.round((ratingForm.kualitasProduk + ratingForm.pelayanan + ratingForm.harga + ratingForm.kebersihan) / 4), 'lg')}
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <textarea
                  value={ratingForm.ulasan}
                  onChange={(e) => setRatingForm({...ratingForm, ulasan: e.target.value})}
                  placeholder="Ceritakan pengalaman Anda... (maks 500 karakter)"
                  maxLength={500}
                  rows={4}
                  className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-yellow-500 resize-none text-sm"
                />
                <div className="text-xs text-gray-500 mt-1 text-right">
                  {ratingForm.ulasan.length}/500 karakter
                </div>
              </div>
            </div>

            {/* Modal Footer */}
            <div className="sticky bottom-0 bg-white border-t-2 border-gray-200 p-4 sm:p-5 md:p-6 rounded-b-3xl flex gap-3">
              <button
                onClick={() => setShowRatingModal(false)}
              ...
              <button
                ...
                disabled={(ratingForm.kualitasProduk + ratingForm.pelayanan + ratingForm.harga + ratingForm.kebersihan) === 0}
                className="flex-2 px-6 py-3 bg-gradient-to-r from-yellow-400 to-orange-500 hover:from-yellow-500 hover:to-orange-600 text-gray-900 font-bold rounded-xl shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
