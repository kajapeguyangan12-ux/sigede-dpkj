...
export default function LembagaPemberdayaanPage() {
  const [lembagaPemberdayaan, setLembagaPemberdayaan] = useState<AnggotaLembaga[]>([]);
    ...
    const fetchData = async () => {
      try {
        ...
        const [dataAnggota, coverImageUrl] = await Promise.all([
          getLembagaKemasyarakatan('pemberdayaan'),
          getLembagaCoverImage('pemberdayaan')
        ]);
        
        setLembagaPemberdayaan(dataAnggota as AnggotaLembaga[]);
  ...
  return (
    <main className="min-h-[100svh] bg-red-50 text-gray-800">
      <div className="w-full px-3 sm:px-4 md:px-6 lg:px-8 xl:px-10 pb-24 sm:pb-28 pt-3 sm:pt-4 md:pt-5 lg:pt-6">
        <HeaderCard 
          title="Lembaga Pemberdayaan Masyarakat" 
          ...
          <div className="rounded-2xl sm:rounded-3xl bg-white/90 backdrop-blur-sm p-4 sm:p-5 md:p-6 lg:p-7 shadow-xl ring-1 ring-red-200">
            ...
            {coverImage ? (
              <div className="rounded-2xl overflow-hidden shadow-inner">
                <img 
                  src={coverImage} 
                  alt="Cover Lembaga Pemberdayaan Masyarakat"
          ...
          <div className="rounded-2xl sm:rounded-3xl bg-white/90 backdrop-blur-sm p-4 sm:p-5 md:p-6 lg:p-7 shadow-xl ring-1 ring-red-200">
            ...
            {isLoading ? (
              ...
              <div className="space-y-3 sm:space-y-4 lg:space-y-5">
                {lembagaPemberdayaan.length > 0 ? (
                  lembagaPemberdayaan.map((item, index) => (
                  ...
                  <div className="text-center py-8">
                    <div className="text-gray-500">Belum ada data lembaga pemberdayaan masyarakat</div>
                  </div>
                )}
              </div>
            )}
