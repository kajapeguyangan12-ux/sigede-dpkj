...
export default function SuratPindahDomisiliPage() {
  ...
  const fetchContent = async () => {
    try {
      setLoading(true);
      const data = await getServiceContent('surat-pindah-domisili');
      setContent(data);
    } catch (error) {
      ...
      <div className="mx-auto w-full max-w-7xl px-3 sm:px-4 md:px-6 lg:px-8 xl:px-10 pb-32 pt-4">
        ...
        <div className="mb-8 text-center">
          <div className="mx-auto mb-4 grid h-20 w-20 sm:h-24 sm:w-24 place-items-center rounded-3xl bg-gradient-to-br from-red-100 via-pink-100 to-red-200 shadow-xl ring-2 ring-red-300">
            <MovingIcon className="h-12 w-12 sm:h-14 sm:w-14 text-red-600" />
          </div>
          <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Surat Pindah Domisili</h2>
