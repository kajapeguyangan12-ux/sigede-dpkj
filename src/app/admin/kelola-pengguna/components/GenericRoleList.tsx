...
export default function GenericRoleList({ roleId, roleLabel }: { roleId: string, roleLabel: string }) {
    ...
    try {
      if (!editing) {
        if (!form.password || form.password.length < 6) {
          setServerError('Password minimal 6 karakter');
          return;
        }
        ...
        try {
          ...
          setServerError(error?.message || 'Gagal membuat pengguna');
        }
      } else {
      ...
      setServerError('Terjadi kesalahan saat menyimpan');
    }
  }

  async function handleDelete(u: unknown) {
    if (!user) { alert('Anda harus login untuk melakukan penghapusan'); return; }
    const userData = u as { id: string; displayName?: string; email: string };
    const ok = confirm(`Hapus pengguna ${userData.displayName || userData.email}?`);
    if (!ok) return;
    try {
  ...
  return (
    <div className="relative">
      <h2 className="text-lg font-semibold text-gray-800 mb-4">List {roleLabel}</h2>
          ...
          <div role="dialog" aria-modal="true" className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={handleSave} className="space-y-3">
              ...
              <div>
                ...
                <input value={roleId} disabled className="mt-1 block w-full border border-gray-200 rounded-md px-3 py-2 bg-gray-100 text-gray-600" />
              </div>
