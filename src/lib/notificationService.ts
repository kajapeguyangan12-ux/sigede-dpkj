...
export interface UniversalNotification {
  id?: string;
  userId: string;
  type: 'pengaduan' | 'layanan_publik' | 'e_umkm_rating';
  referenceId: string; // ID of the related pengaduan, layanan, or umkm
  title: string;
  message: string;
  status: 'unread' | 'read';
...
// Create notification for E-UMKM rating reminder
export const createRatingReminderNotification = async (data: {
  userId: string;
  umkmId: string;
  umkmName: string;
  priority?: 'low' | 'medium' | 'high';
  ...
  try {
    const notification: Omit<UniversalNotification, 'id'> = {
      userId: data.userId,
      type: 'e_umkm_rating',
      referenceId: data.umkmId,
      title: 'Bagaimana pengalaman Anda?',
      message: `Anda pernah mengunjungi ${data.umkmName}. Yuk, beri rating dan ulasan untuk membantu warga lain!`,
      status: 'unread',
    ...
    const docRef = await addDoc(collection(db, COLLECTION_NOTIFICATIONS), notification);
    console.log('✅ E-UMKM rating reminder notification created:', docRef.id);
    return docRef.id;
  } catch (error) {
